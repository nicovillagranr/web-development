...
function WizardModal({ isOpen, onClose, form, error, setField, addItem, resetForm }) {
    ...
    // Ref para evitar stale closure en el keydown handler
    ...
    return (
        ...
        >
            ...
            {/* Panel */}
            <div className="wizard-enter relative w-full rounded-t-2xl bg-(--glass-dark) border border-(--glass-border) border-b-0 backdrop-blur-xl px-5 pt-5 pb-6 z-10">
                    ...
                    {step === 1 && (
                        <div>
                            ...
                            <input
                                ...
                                autoComplete="off"
                                className="w-full h-11 rounded-xl bg-white/7 border border-white/8 px-3 text-sm text-white placeholder:text-white/25 outline-none focus:border-green-400/40 transition-colors"
                            />
                        </div>
                    )}
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full h-11 rounded-xl bg-white/7 border border-white/8 px-3 text-sm text-white/80 outline-none focus:border-green-400/40 transition-colors"
                                >
                                    {INVENTORY_UNITS.map((u) => (
                                        <option className="bg-[#0a0c14] text-white" key={u.value} value={u.value}>
                                            {u.label}
                                        </option>
                                    ))}
                                </select>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                >
                                    {INVENTORY_TYPES.map((t) => (
                                        <option className="bg-[#0a0c14] text-white" key={t.value} value={t.value}>
                                            {t.label}
                                        </option>
                                    ))}
                                </select>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full h-11 rounded-xl bg-white/7 border border-white/8 px-3 text-sm text-white/70 outline-none focus:border-green-400/40 transition-colors"
                                />
                            </div>
                        </div>
                    )}
                ...
                <div className="flex gap-2 mt-5">
                    <button
                        type="button"
                        onClick={handleClose}
                        className="h-10 px-4 rounded-xl bg-white/5 border border-white/7 text-sm text-white/40 hover:text-white/65 hover:bg-white/8 transition-colors"
                    >
                        Cancelar
                    </button>
                    <button
                        type="button"
                        onClick={handleNext}
                        className="flex-1 h-10 rounded-xl bg-green-500/15 border border-green-400/25 text-sm text-green-300 hover:bg-green-500/20 hover:border-green-400/35 transition-colors"
                    >
