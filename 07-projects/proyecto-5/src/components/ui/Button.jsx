...
export default function Button({ children, href, variant = 'primary', size = 'md', className = '', onClick }) {
  // Clases base compartidas por todas las variantes y tamaños
  ...
  // Tamaños disponibles: md (navbar/cards) y lg (hero/cta)
  const sizes = {
  ...
  // Variantes visuales:
  // primary → gradiente magenta/sunset con glow rosa
  // outline → borde y texto cyan con glow azul
  const variants = {
    primary:
  ...
  // Si tiene href, renderiza como <a> (link externo); si no, como <button>
  if (href) {
    return (
