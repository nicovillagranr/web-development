...
function TimeSettings({
    ...
    return (
        <section className={`absolute inset-0 z-20 py-4 bg-white flex flex-col transition-transform duration-500 ease-out ${isActive ? "translate-x-0" : "-translate-x-full"}`}>

            {/* Header del panel */}
            <header className="h-14 flex items-center px-4">
                <button aria-label="Volver" className="w-8 h-8 flex items-center justify-center rounded-full transition" onClick={() => {
                    if (activeEditor) {
                        setActiveEditor(null);
                    } else {
                        onBack();
            ...
            <div className="flex-1 p-4 space-y-2">
                ...
                <div className="h-14 rounded-lg bg-gray-100 flex items-center justify-between px-4">
                    ...
                    <button onClick={() => setAutoTime(!autoTime)} className={`relative w-11 h-6 rounded-full transition ${autoTime ? "bg-green-500" : "bg-gray-300"}`}>
                        <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${autoTime ? "translate-x-5" : ""}`} />
                    </button>
                </div>

                {/* Ajustar Fecha */}
                <div onClick={() => setActiveEditor("date")} className={`h-14 rounded-lg bg-gray-100 flex items-center px-4 ${autoTime
                ...
                {/* Ajustar Hora */}
                <div onClick={() => setActiveEditor("time")}
                ...
                <div className="h-14 rounded-lg bg-gray-100 flex items-center justify-between px-4 mt-2">
                    ...
                    <button onClick={() => setIs24hFormat(!is24hFormat)} className={`relative w-11 h-6 rounded-full transition ${is24hFormat ? "bg-green-500" : "bg-gray-300"}`}>
