...
**REFUERZO ACTIVO (13 jun 2026): `07-utility-types/exercise-03` — "el campeón, del bucle al reduce".** A petición suya ("necesito más ejercicios para entender la lógica del drill 7"). 6 drills, mismo problema resuelto 2 veces: Bloque A (1–3, con `for`: mayorBucle/menorBucle/masLargaBucle — completa la coronación/inicial), Bloque B (4–6, el MISMO con `reduce`, con el bucle al lado como referencia: mayorReduce/menorReduce/masLargaReduce). Estado inicial verificado: 6 tests rojos + typecheck rojo temporal (TS6133 `valor` sin usar, se limpia al rellenar). **Retomar por el drill 1 (`mayorBucle`).** Al terminar, volver a exercise-02 a rematar 7→13. PENDIENTE de razonamiento (lo ha esquivado 4 veces): articular por qué el inicial de un mínimo es `Infinity` y no `0`.
