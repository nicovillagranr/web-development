...
interface Persona {
  ...
  edad: number;
  ciudad: string;
  profesion?: string; // propiedad opcional con '?'
}
...
// Array tipado
// -----------------------------------------------------------------------------------
const cajaFrutas: string[] = ["Pera", "Manzana", "Sandía", "Mangos"];
...
console.log(
  `El nombre de mi PC es: ${pc.nombre}, procesador: ${pc.procesador}, RAM: ${pc.ram}, almacenamiento: ${pc.espacio}`
);
...
for (const prop in IPhone) {
  const key = prop as keyof typeof IPhone;
...
console.log(
  `Mi juego favorito es: ${TLOU.nombre}. Año: ${TLOU.año}, Género: ${TLOU.genero}, Plataforma: ${TLOU.plataforma}, Jugadores: ${TLOU.jugadores}`
);
...
const pilotos: string[] = ["Leclerc", "Sainz", "Verstappen"];
const nuevosPilotos: string[] = ["Piastri", "Norris"];
const todosPilotos: string[] = [...pilotos, ...nuevosPilotos]; // tipo string[] infierido
console.log(todosPilotos);

// Spread con objetos tipados
const coche1: Coche = { marca: "Ferrari", motor: "V6" };
const coche2: Coche = { ...coche1, color: "Rojo" }; // color es propiedad opcional
console.log(coche2);
