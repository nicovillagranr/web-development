...
const definirCompra = (nombre) => {
    const input = prompt(`Monto que tiene ${nombre}`);
    if (input === null)
        return `${nombre}: Canceló la consulta<br>`;
    const dinero = parseFloat(input);
    if (isNaN(dinero))
        return `${nombre}: Valor inválido<br>`;
    if (dinero >= 0.6 && dinero < 1)
        return `${nombre}: Helado de agua<br>`;
    if (dinero >= 1 && dinero < 1.6)
        return `${nombre}: Helado de crema<br>`;
    if (dinero >= 1.6 && dinero < 1.7)
        return `${nombre}: Helado de Heladix<br>`;
    if (dinero >= 1.7 && dinero < 1.8)
        return `${nombre}: Helado de Heladovich<br>`;
    if (dinero >= 1.8 && dinero < 2.9)
        return `${nombre}: Helado de Helardo<br>`;
    if (dinero >= 2.9)
        return `${nombre}: Helado de confites o pote de 1/4 Kg<br>`;
...
function compra(dinero, precio) {
    const cantidad = Math.floor(dinero / precio);
    const vuelto = dinero % precio;
    if (cantidad === 0) {
...
export {};
//# sourceMappingURL=cofla_1.js.map