...
export function cantidadItems(p: Pedido): number {
  // completa aquí (estrecha por p.estado; "cancelado" no tiene items)
  return 0
}
...
//    pagar({ estado: "carrito", items: ["a"] }, "tarjeta")
//      → { ok: true, valor: { estado: "pagando", items: ["a"], metodo: "tarjeta" } }
export function pagar(p: Pedido, metodo: "tarjeta" | "efectivo"): Resultado<Pedido> {
  // completa aquí (estrecha por p.estado; transición válida solo desde "carrito")
...
//    confirmar({ estado: "pagando", items: ["a"], metodo: "efectivo" }, 1234)
//      → { ok: true, valor: { estado: "confirmado", items: ["a"], numero: 1234 } }
