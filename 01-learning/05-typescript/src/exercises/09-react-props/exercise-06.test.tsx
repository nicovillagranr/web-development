...
describe("09-react-props / exercise-06 — interface, extends y tipos derivados", () => {
  ...
  it("4) Chip — junta className, etiqueta y onQuitar de dos bases", () => {
    const quitar = vi.fn();
    const { container } = render(<Chip className="tag" etiqueta="react" onQuitar={quitar} />);
  ...
  it("10) TarjetaProducto — clase normal y botón que entrega el callback", () => {
    const comprar = vi.fn();
    const { container } = render(<TarjetaProducto nombre="Pan" precio={2} onComprar={comprar} />);
    const article = container.querySelector("article");
    expect(article?.getAttribute("class")).toBe("normal");
    expect([...(article?.children ?? [])].map((el) => el.tagName.toLowerCase())).toEqual([
      "h3",
      "b",
      "button",
    ]);
