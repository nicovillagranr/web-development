...
describe("09-react-props / exercise-09 — props-función con argumentos", () => {
  ...
  it("8b) Precio — quien llama decide el formato; el componente no sabe nada", () => {
    const { container } = render(<Precio valor={12} formatear={(n) => `USD ${n.toFixed(2)}`} />);
  ...
  const etiquetaDe: ListaTareasProps["etiquetaDe"] = (t) => (t.hecha ? `✔ ${t.titulo}` : t.titulo);
  ...
  it("10) ListaTareas — lista vacía, estado vacío", () => {
    render(<ListaTareas tareas={[]} onAlternar={() => {}} etiquetaDe={etiquetaDe} />);
  ...
  it("10c) ListaTareas — cada botón alterna SU tarea", () => {
    ...
    render(<ListaTareas tareas={TAREAS} onAlternar={alternar} etiquetaDe={etiquetaDe} />);
  ...
  it("10d) ListaTareas — sin onBorrar, el botón Borrar existe y no revienta", () => {
    render(<ListaTareas tareas={TAREAS} onAlternar={() => {}} etiquetaDe={etiquetaDe} />);
